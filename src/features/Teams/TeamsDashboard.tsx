...
import { DataManager, CATEGORY_ORDER, Category, Team, getCurrentSeasonYear, getSeasonLabel } from "../../lib/dataManager";
...
export default function TeamsDashboard() {
  ...
  // ── PRÉPARER SAISON PROCHAINE ──
  const handlePrepareNextSeason = () => {
    const currentSeason = getSeasonLabel(getCurrentSeasonYear());
    const nextSeason = getSeasonLabel(getCurrentSeasonYear() + 1);

    const msg = [
      `Préparer la saison ${nextSeason} :\n`,
      `• Les catégories de toutes les joueuses seront recalculées depuis leur date de naissance pour la saison ${nextSeason}.`,
      `• Les ${teams.length} équipe(s) seront supprimées.`,
      `\nPensez à faire un Backup avant ! Continuer ?`,
    ].join("\n");

    if (!window.confirm(msg)) return;

    const result = DataManager.prepareNextSeason();
    alert(`Saison ${result.seasonLabel} préparée !\n${result.updatedCount} catégorie(s) mise(s) à jour.`);
    refresh();
  };
    ...
    <div className="h-full flex flex-col p-4 gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-none">
        <h1 className="text-xl font-black uppercase italic tracking-tighter text-white">
          ...
          <span className="text-slate-500 text-sm ml-2">{teams.length} créée(s) · Saison {seasonLabel}</span>
        </h1>
        <div className="flex gap-2">
          ...
          <button onClick={handlePrepareNextSeason}
            className="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest bg-zinc-800 text-slate-300 hover:bg-zinc-700 border border-zinc-700"
            title="Recalculer les catégories depuis les dates de naissance pour la saison prochaine">
            🗓 Préparer saison prochaine
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 min-h-0">
        {/* Pool gauche */}
        <div onDragOver={handlePoolDragOver} onDragLeave={() => setPoolRing(false)} onDrop={handlePoolDrop}
          ...
          <div className="flex items-center justify-between mb-3 flex-none">
            ...
            <select value={filterCat} onChange={(e) => setFilterCat(e.target.value as "ALL" | Category)}
        ...
        {/* Équipes droite */}
        <div className="overflow-auto flex flex-col gap-4 pr-1">
          {teams.length === 0 ? (
            ...
            teams.map((t) => (
              <TeamCard key={t.id} team={t}
                ...
                onDropPlayer={(pid) => handleDropOnTeam(pid, t.id)}
